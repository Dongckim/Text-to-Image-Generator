...
def contrastive_loss_G(fake_image, clip_model, txt_embedding, device, tau=0.5):
    ...
    image_features = normalize(image_features)  # Normalize the image features

    # Calculate cosine similarity
    sim = cos_sim(image_features, txt_embedding)
    
    # Calculate contrastive loss L_ConG
    numerator = torch.exp(sim / tau)
    denominator = torch.sum(torch.exp(sim / tau))
    L_cont = -tau * torch.sum(torch.log(numerator / denominator))

    # params = sim/tau
    # L_cont = -tau * torch.sum(torch.log(torch.nn.functional.softmax(params)))
...
def contrastive_loss_D(g_out_align, txt_embedding, tau=0.5):
    ...
    model_features = normalize(g_out_align)  # Normalize the discriminator's output features
    
    # Calculate cosine similarity
    sim = cos_sim(model_features, txt_embedding)
    
    # Calculate contrastive loss L_ConD
    numerator = torch.exp(sim / tau)
    denominator = torch.sum(torch.exp(sim / tau))
    L_cont = -tau * torch.sum(torch.log(numerator / denominator))
...
def D_loss(real_image, fake_image, model_D, loss_fn, 
    ...
    if use_uncond_loss:
        # (1) Unconditional loss with fake images
        d_out_fake_uncond, d_out_align_fake_uncond  = model_D(fake_image)
        loss_d_comp['d_loss_fake_uncond'] = loss_fn(torch.squeeze(d_out_fake_uncond), d_fake_label)
        # (2) Unconditional loss with real images
        d_out_real_uncond, d_out_align_real_uncond = model_D(real_image)
        loss_d_comp['d_loss_real_uncond'] = loss_fn(torch.squeeze(d_out_real_uncond), d_real_label)

    # (3) Conditional loss with fake images
    d_out_fake, d_out_align_fake = model_D(fake_image, condition=mu)
    loss_d_comp['d_loss_fake_cond'] = loss_fn(torch.squeeze(d_out_fake), d_fake_label)

    # (4) Conditional loss with real images
    d_out_real, d_out_align_real = model_D(real_image, condition=mu)
    loss_d_comp['d_loss_real_cond'] = loss_fn(torch.squeeze(d_out_real), d_real_label)

    # (5) Contrastive loss (if applicable)
...
def G_loss(fake_image, model_D, loss_fn,
    ...
    if use_uncond_loss:
        g_out_uncond, g_out_align_uncond = model_D(fake_image)
        loss_g_comp['g_loss_uncond'] = loss_fn(torch.squeeze(g_out_uncond), g_label)

    # (2) Conditional loss
    g_out_cond, g_out_align_cond = model_D(fake_image, condition=mu)
    loss_g_comp['g_loss_cond'] = loss_fn(torch.squeeze(g_out_cond), g_label)

    # (3) Contrastive loss (if applicable)
    g_out_align = g_out_align_cond
    if use_contrastive_loss:
...
def train_step(train_loader, noise_dim, device, model_G, model_D_lst, optim_g, optim_d_lst, 
    ...
    for iter, batch in enumerate(train_loader):
        ...
        noise = 0.1 * torch.randn_like(img_feature)  # Assuming a fixed perturbation of 0.1 as an example
        pseudo_text_feature = img_feature + noise
        pseudo_text_feature = normalize(pseudo_text_feature)
        ...
        d_fake_label = torch.zeros(BATCH_SIZE, dtype=torch.float32).to(device)
        d_real_label = torch.ones(BATCH_SIZE, dtype=torch.float32).to(device)
        g_label = torch.ones(BATCH_SIZE, dtype=torch.float32).to(device)
        ################# Problem 4-(e). #################
